...
esp_err_t baseline_init(csi_baseline_t *bf) {
    ...
    ESP_LOGI(TAG, "Baseline 재학습 시작, 시작 메시지 송신 / topic : %s", WIFY_TOPIC("/baseline/status"));
    mqtt_publish(WIFY_TOPIC("/baseline/status"), "MEASURING", 1, 3);
    return ESP_OK;
}
...
void baseline_update(csi_baseline_t *bf, const float *amp) {
    ...
    if (bf->sample_count >= BASELINE_CALIB_FRAMES) {
        ...
        ESP_LOGI(TAG, "Baseline 캘리브레이션 완료, 완료 메시지 송신 / topic : %s", WIFY_TOPIC("/baseline/status"));
        mqtt_publish(WIFY_TOPIC("/baseline/status"), "DONE", 1, 3);
